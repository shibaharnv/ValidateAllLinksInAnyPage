...
public class ImageValidationJavaScriptFunction {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            writer.write("<table border='1'><tr><th>Application Name</th><th>Page link</th><th>Total Images Count</th><th>Missing Images Count</th><th>Missing Images List</th></tr>");
    ...
    private static int getMissingImagesCount(String[] imagesArray) {
        ...
        for (String imageUrlToCheck : imagesArray) {
            if (!isImageLoaded(imageUrlToCheck)) {
                missingImagesCount++;
            }
        }
    ...
    private static boolean isImageLoaded(String imageUrl) {
        JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;
        try {
            // Execute JavaScript to check if the image is loaded
            ...
            // Handle any exceptions, e.g., if JavaScript execution fails
            return false;
        }
    }
    ...
    private static void ImageValidationReport(BufferedWriter writer, String applicationName, String applicationUrl, int totalImagesCount, int missingImagesCount, List<String> missingImagesList) {
        try {
            ...
            writer.write("<td>");
            if (missingImagesList.isEmpty()) {
                writer.write("No missing images");
            } else {
...
//    }
    ...
    private static void writeLinksList(BufferedWriter writer, List<String> linksList) throws IOException {
        for (String notWorkingImage : linksList) {
            writer.write("<a href='" + notWorkingImage + "'>" + notWorkingImage + "</a><br>");
        }
    }
}